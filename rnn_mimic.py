...
from tensorflow.keras.layers import Masking, Flatten, Embedding, Dense, LSTM, TimeDistributed, Input
...
def build_model(no_feature_cols=None, time_steps=7, output_summary=False):
    ...
    # Apply masking to ignore padding values
    x = Masking(mask_value=0, input_shape=(time_steps, no_feature_cols))(input_layer)

    # Attention layer (custom implementation or library)
    x = Attention(x, time_steps)  # Ensure Attention is defined and works with masking
    ...
    # Compile the model
    RMS = RMSprop(learning_rate=0.001, rho=0.9, epsilon=1e-08)
...
def train(model_name="mimic3", synth_data=False, target='MI',
  ...
  timestamp = time.time()  # Current time as a float
  log_dir = './logs/{0}_{1:.0f}'.format(model_name, timestamp).replace(':', '_')  # Format the log directory path
  ...
  # Make checkpoint directory and initialize the ModelCheckpoint callback
  checkpoint_dir = f"./saved_models/{model_name}"
  ...
  print(f"X_TRAIN shape: {X_TRAIN.shape}")  # Ensure shape is (batch_size, time_steps, feature_dim)
  if X_TRAIN.shape[0] == 0 or Y_TRAIN.shape[0] == 0:
      raise ValueError("Training data is empty. Please check the data loading process.")
  ...
  model.fit(
      ...
      shuffle=True,
      steps_per_epoch=len(X_TRAIN) // 16  # Optional, if dataset size is known
  )
...
def return_loaded_model(model_name="mimic3_0"):
...
## BIG THREE ##
    ...
    K.clear_session()
    train(model_name='mimic3_final_no_mask_MI_pad14', epochs=13,
          synth_data=False, predict=True, target='MI', time_steps=14)

    K.clear_session()

    train(model_name='mimic3_final_no_mask_VANCOMYCIN_pad14', epochs=14,
          synth_data=False, predict=True, target='VANCOMYCIN', time_steps=14) 

    K.clear_session()

    train(model_name='mimic3_final_no_mask_SEPSIS_pad14', epochs=17,
...
## MI ##
    ...
    train(model_name='mimic3_final_no_mask_MI_pad14_80_percent', epochs=13,
          synth_data=False, predict=True, target='MI', time_steps=14,
          n_percentage=0.80)
  
    K.clear_session()
  
    train(model_name=',i,ic3_final_no_mask_MI_pad14_60_percent', epochs=13,
          synth_data=False, predict=True, target='MI', time_steps=14,
          n_percentage=0.60)
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_MI_pad14_40_percent', epochs=13,
          synth_data=False, predict=True, target='MI', time_steps=14,
          n_percentage=0.40)
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_MI_pad14_20_percent', epochs=13,
          synth_data=False, predict=True, target='MI', time_steps=14,
          n_percentage=0.20)
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_MI_pad14_10_percent', epochs=13,
          synth_data=False, predict=True, target='MI', time_steps=14,
          n_percentage=0.10)
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_MI_pad14_5_percent', epochs=13,
          synth_data=False, predict=True, target='MI', time_steps=14,
          n_percentage=0.05)
...
# SEPSIS ##
    ...
    train(model_name='mimic3_final_no_mask_VANCOMYCIN_pad14_80_percent',
          epochs=14,synth_data=False, predict=True, target='VANCOMYCIN',
          time_steps=14, n_percentage=0.80) 
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_VANCOMYCIN_pad14_60_percent',
          epochs=14, synth_data=False, predict=True, target='VANCOMYCIN',
          time_steps=14, n_percentage=0.60) 
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_VANCOMYCIN_pad14_40_percent',
          epochs=14, synth_data=False, predict=True, target='VANCOMYCIN',
          time_steps=14, n_percentage=0.40) 
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_VANCOMYCIN_pad14_20_percent', epochs=14,
          synth_data=False, predict=True, target='VANCOMYCIN', time_steps=14,
          n_percentage=0.20) 
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_VANCOMYCIN_pad14_10_percent',
          epochs=13, synth_data=False, predict=True, target='VANCOMYCIN',
          time_steps=14, n_percentage=0.10)
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_VANCOMYCIN_pad14_5_percent',
          epochs=13, synth_data=False, predict=True, target='VANCOMYCIN',
          time_steps=14, n_percentage=0.05)
 
# VANCOMYCIN ##
 
    train(model_name='mimic3_final_no_mask_SEPSIS_pad14_80_percent',
          epochs=17, synth_data=False, predict=True, target='SEPSIS',
          time_steps=14, n_percentage=0.80) 
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_SEPSIS_pad14_60_percent',
          epochs=17, synth_data=False, predict=True, target='SEPSIS',
          time_steps=14, n_percentage=0.60) 
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_SEPSIS_pad14_40_percent',
          epochs=17, synth_data=False, predict=True, target='SEPSIS',
          time_steps=14, n_percentage=0.40) 
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_SEPSIS_pad14_20_percent',
          epochs=17, synth_data=False, predict=True, target='SEPSIS',
          time_steps=14, n_percentage=0.20) 
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_SEPSIS_pad14_10_percent',
          epochs=13, synth_data=False, predict=True, target='SEPSIS',
          time_steps=14, n_percentage=0.10)
  
    K.clear_session()
  
    train(model_name='mimic3_final_no_mask_SEPSIS_pad14_5_percent',
          epochs=13, synth_data=False, predict=True, target='SEPSIS',
          time_steps=14, n_percentage=0.05)
