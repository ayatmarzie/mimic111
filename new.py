...
class MimicParser(object):
    ...
    def add_icd_infect(self, file_name):
        ...
        # Load the ICD procedures and microbiology events data
        df_icd = pd.read_csv('./mimic_database/PROCEDURES_ICD.csv')
        df_micro = pd.read_csv('./mimic_database/MICROBIOLOGYEVENTS.csv')

        # Extract the set of suspected HADM_IDs from the microbiology data
        self.suspect_hadmid = set(pd.unique(df_micro['HADM_ID']).tolist())

        # Load the ICD9 codes and their counts from the text file
        icd9_counts_df = pd.read_csv('the icd9s.txt', sep='\t')
        print(icd9_counts_df.head)

        # Filter the ICD9 codes with count greater than 100
        icd9_codes_to_process = icd9_counts_df[icd9_counts_df['Count'] > 100]['ICD9_CODE']

        # Iterate over the filtered ICD9 codes and process them
        for icd9_code in icd9_codes_to_process:
            print(f"Processing ICD9_CODE: {icd9_code}")

            # Filter the ICD data to include only rows for the current ICD9_CODE
            df_icd_ckd = df_icd[df_icd['ICD9_CODE'] == icd9_code]

            # Create a set of HADM_IDs from the filtered DataFrame
            self.ckd = set(df_icd_ckd['HADM_ID'].values.tolist())

            # Load the target file and add CKD and Infection columns
            df = pd.read_csv(file_name)
            df['ICD9_CODE'] = df['HADM_ID'].apply(lambda x: icd9_code if x in self.ckd else 0)
            df['Infection'] = df['HADM_ID'].apply(lambda x: 1 if x in self.suspect_hadmid else 0)

            # Save the modified DataFrame with the current ICD9_CODE
            df.to_csv(file_name[0:-4] + f'_plus_icds_{icd9_code}.csv', index=False)

    def add_notes(self, file_name):
        unique_words_file = 'unique_words.txt'
        ...
        with open(unique_words_file, 'r') as file:
            unique_words = [line.strip() for line in file.readlines()]

        # Create new columns for each unique word
        for word in unique_words:
            word_bool_array = df_rad_notes['TEXT'].str.contains(word, flags=re.IGNORECASE)
            word_hadm_ids = np.unique(df_rad_notes['HADM_ID'][word_bool_array].dropna())
            word_hadm_id_set = set(word_hadm_ids.tolist())

            # Add a column for this word in df2
            df2[word] = df2['HADM_ID'].apply(lambda x: 1 if x in word_hadm_id_set else 0)

            # Step 7: Save the updated file
        output_file_name = file_name[0:-4] + '_plus_notes.csv'
...
if __name__ == '__main__':
    ...
    mp = MimicParser()
    # mp.save_unique_icd9_codes("the icd9s.txt")
    #
    ...
    mp.add_notes(
        ROOT + FOLDER + FILE_STR + '_24_hour_blocks_plus_admissions_plus_patients_plus_scripts_plus_icds_9604.csv')

    # df=pd.read_csv(ROOT + FOLDER + FILE_STR + '_24_hour_blocks.csv')
    # for i in df:
    #     print(i)
    #
