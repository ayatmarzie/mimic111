...
class MimicParser(object):
    ...
    def group_icd9_codes(self,file_path):
        df = pd.read_csv(file_path)

        # Ensure ICD9_CODE is string for consistency
        df['ICD9_CODE'] = df['ICD9_CODE'].astype(str)

        # Get all columns except ICD9_CODE
        agg_dict = {col: 'first' for col in df.columns if col not in ['HADM_ID', 'ICD9_CODE']}
        # Add aggregation rule for ICD9_CODE
        agg_dict['ICD9_CODE'] = lambda x: list(set(x.dropna()))

        # Group by HADM_ID
        df_grouped = df.groupby('HADM_ID', as_index=False).agg(agg_dict)

        # Save the result
        output_file = file_path.replace('.csv', '_grouped.csv')
        df_grouped.to_csv(output_file, index=False)
        print(f"Grouped file saved to {output_file}")
...
if __name__ == '__main__':
   ...
   # mp.add_icd_infect(ROOT + FOLDER + FILE_STR + '_24_hour_blocks_plus_admissions_plus_patients_plus_scripts.csv')
    mp.group_icd9_codes(ROOT  + FILE_STR + '_24_hour_blocks_plus_admissions_plus_patients_plus_scripts_plus_icds.csv')
