...
#                df2[i] = df2[i].where(df2[i] > df2[i].quantile(.875)).fillna(df2[i].median())
    ...
    def add_icd_infect(self, file_name):
        ...
        # Select only HADM_ID and ICD9_CODE columns to simplify further processing
        df_icd_subset = df_icd[['HADM_ID', 'ICD9_CODE']].drop_duplicates()

        # Print the first few rows to confirm the data
        print("ICD9_CODE subset preview:")
        print(df_icd_subset.head())

        # Load the input file
        df = pd.read_csv(file_name)

        # Merge the ICD9_CODE information into the input file based on HADM_ID
        df = df.merge(df_icd_subset, on='HADM_ID', how='left')

        # Add Infection column based on HADM_IDs in microbiology data
    ...
    def add_icd_infect(self, file_name):
        ...
        # Merge the ICD9_CODE information into the input file based on HADM_ID
        df = df.merge(df_icd_subset, on='HADM_ID', how='left')
        #to avoid duplicating rows on icd9
        df_icd_grouped = df_icd_subset.groupby('HADM_ID')['ICD9_CODE'].apply(
        ...
        # Save the updated DataFrame to a new file
        output_file = file_name[0:-4] + '_plus_icds.csv'
        output_file1 = file_name[0:-4] + '_plus_icds_notduplicated.csv'
        df.to_csv(output_file, index=False)
        df_icd_grouped.to_csv(output_file1, index=False)
        print(f"Output saved to {output_file}")
    ...
    def add_notes(self,file_name):
        ...
        for word in unique_words:
            word_bool_array = df_rad_notes['TEXT'].str.contains(word, flags=re.IGNORECASE, na=False)
            df_rad_notes[word] = word_bool_array.astype(int)  # Add as a binary column (1 for present, 0 for absent)

        # Save the updated DataFrame to a new file or return it
        updated_file_name = "updated_" + file_name
        df_rad_notes.to_csv(updated_file_name, index=False)
